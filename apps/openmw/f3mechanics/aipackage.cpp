...
namespace
{
    ...
    float getPointTolerance(float speed, float duration, const osg::Vec3f& halfExtents)
    {
        ...
        return std::max(F3Mechanics::MIN_TOLERANCE, actorTolerance);
    }
...
F3Mechanics::AiPackage::AiPackage(F3Mechanics::AiPackageTypeId typeId, const F3Mechanics::AiPackage::Options &options, std::string name) :
    mTypeId(typeId),
    mOptions(options),
...
MWWorld::Ptr F3Mechanics::AiPackage::getTarget() const
{
    ...
    if (mTargetActorId == -1)
    {
        if (mTargetActorRefId == 0)
        {
            mTargetActorId = -2;
            return MWWorld::Ptr();
        }
        mCachedTarget = MWBase::Environment::get().getWorld()->searchPtrViaFormId(mTargetActorRefId);
        if (mCachedTarget.isEmpty())
        {
            mTargetActorId = -2;
            return mCachedTarget;
            ...
            mTargetActorId = mCachedTarget.getClass().getFOStats(mCachedTarget).mActorId;
    }
...
void F3Mechanics::AiPackage::reset()
{
    // reset all members
    mReaction.reset();
    mIsShortcutting = false;
...
bool F3Mechanics::AiPackage::pathTo(const MWWorld::Ptr &actor, const osg::Vec3f &dest, float duration, float destTolerance, float endTolerance, F3Mechanics::PathType pathType)
{
    ...
    const bool actorCanMoveByZ = MWMechanics::canActorMoveByZAxis(actor);
    ...
    if (!isDestReached && timerStatus == Misc::TimerStatus::Elapsed)
    {
        ...
        if (!mIsShortcutting)
        {
            ...
            {
                // todo: limited path without pathgrids (fo3 doesn't have pathgrids)
    ...
    if (isDestReached || mPathFinder.checkPathCompleted()) // if path is finished
    {
        ...
        MWMechanics::zTurn(actor, getZAngleToPoint(position, dest));
        MWMechanics::smoothTurn(actor, getXAngleToPoint(position, dest), 0);
    ...
    MWMechanics::zTurn(actor, zAngleToNext);
    MWMechanics::smoothTurn(actor, mPathFinder.getXAngleToNext(position.x(), position.y(), position.z()), 0);
...
void F3Mechanics::AiPackage::evadeObstacles(const MWWorld::Ptr &actor)
{
    ...
    const MWWorld::Ptr door = MWMechanics::getNearbyDoor(actor, distance);
...
void F3Mechanics::AiPackage::openDoors(const MWWorld::Ptr &actor)
{
...
const ESM4::NavMesh *F3Mechanics::AiPackage::getNavMesh(const MWWorld::CellStore *cell)
{
    assert(cell->isTes4());
    const ESM4::FormId id = cell->getCell4()->mFormId;
    typedef std::map<ESM4::FormId, const ESM4::NavMesh*> CacheMap;
    static CacheMap cache;
    CacheMap::iterator it = cache.find(id);
    if (it == cache.end())
    {
        MWBase::World* world = MWBase::Environment::get().getWorld();
        for (const auto& navi : world->getStore().get<ESM4::Navigation>())
        {
            for (const auto& info : navi.mNavMeshInfoFO3)
            {
                ...
                {
                    if (const auto navm = world->getStore().get<ESM4::NavMesh>().search(info.navMesh))
                    {
...
bool F3Mechanics::AiPackage::shortcutPath(const osg::Vec3f &startPoint, const osg::Vec3f &endPoint, const MWWorld::Ptr &actor, bool *destInLOS, bool isPathClear)
{
...
bool F3Mechanics::AiPackage::checkWayIsClearForActor(const osg::Vec3f &startPoint, const osg::Vec3f &endPoint, const MWWorld::Ptr &actor)
{
    if (MWMechanics::canActorMoveByZAxis(actor))
        return true;

    const float actorSpeed = actor.getClass().getMaxSpeed(actor);
    const float maxAvoidDist = MWMechanics::AI_REACTION_TIME * actorSpeed + actorSpeed / MWMechanics::getAngularVelocity(actorSpeed) * 2; // *2 - for reliability
...
bool F3Mechanics::AiPackage::doesPathNeedRecalc(const osg::Vec3f &newDest, const MWWorld::Ptr &actor) const
{
...
bool F3Mechanics::AiPackage::isNearInactiveCell(osg::Vec3f position)
{
    const ESM4::Cell* playerCell = MWMechanics::getPlayer().getCell()->getCell4();
    if (!playerCell)
        return false;
...
bool F3Mechanics::AiPackage::isReachableRotatingOnTheRun(const MWWorld::Ptr &actor, const osg::Vec3f &dest)
{
    ...
    float radius = speed / MWMechanics::getAngularVelocity(speed);
...
DetourNavigator::Flags F3Mechanics::AiPackage::getNavigatorFlags(const MWWorld::Ptr &actor) const
{
    ...
    if ((actorClass.isPureWaterCreature(actor)
         || (getTypeId() != AiPackageTypeId::Wander
             && ((allowToFollowOverWaterSurface && getTypeId() == AiPackageTypeId::Follow)
                 ...
                 || MWMechanics::hasWaterWalking(actor)))
...
DetourNavigator::AreaCosts F3Mechanics::AiPackage::getAreaCosts(const MWWorld::Ptr &actor) const
{
...
osg::Vec3f F3Mechanics::AiPackage::getNextPathPoint(const osg::Vec3f &destination) const
{
    return mPathFinder.getPath().empty() ? destination : mPathFinder.getPath().front();
}

float F3Mechanics::AiPackage::getNextPathPointTolerance(float speed, float duration, const osg::Vec3f &halfExtents) const
{
