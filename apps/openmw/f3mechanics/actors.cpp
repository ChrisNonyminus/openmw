...
bool isConscious(const MWWorld::Ptr& ptr)
{
    ...
    return !stats.mDead && !stats.mKnockdown;
}

// used by skill calculation
static const F3Mechanics::SPECIAL statSkillUses[] =
{
    F3Mechanics::Stat_Charisma, // barter
    F3Mechanics::Stat_Endurance, // big guns
    F3Mechanics::Stat_Perception, // energy weapons
    F3Mechanics::Stat_Perception, // explosives
    F3Mechanics::Stat_Perception, // lockpick
    F3Mechanics::Stat_Intelligence, // medicine
    F3Mechanics::Stat_Strength, // melee weapons
    F3Mechanics::Stat_Intelligence, // repair
    F3Mechanics::Stat_Intelligence, // science
    F3Mechanics::Stat_Agility, // guns/smallguns
    F3Mechanics::Stat_Agility, // sneak
    F3Mechanics::Stat_Charisma, // speech
    F3Mechanics::Stat_Endurance, // survival/throwing(?)
    F3Mechanics::Stat_Endurance, // unarmed
};

static const std::string skillAVDSettingNV[] =
{
    "fAVDSkillBarterBase", // barter
    "", // big guns
    "fAVDSkillEnergyWeaponsBase", // energy weapons
    "fAVDSkillExplosivesBase", // explosives
    "fAVDSkillLockpickBase", // lockpick
    "fAVDSkillMedicineBase", // medicine
    "fAVDSkillMeleeWeaponsBase", // melee weapons
    "fAVDSkillRepairBase", // repair
    "fAVDSkillScienceBase", // science
    "fAVDSkillGunsBase", // guns/smallguns
    "fAVDSkillSneakBase", // sneak
    "fAVDSkillSpeechBase", // speech
    "fAVDSkillSurvivalBase", // survival/throwing(?)
    "fAVDSkillUnarmedBase", // unarmed
};

static const std::string skillAVDSettingFO3[] =
{
    "fAVDSkillBarterBase", // barter
    "fAVDSkillBigGunsBase", // big guns
    "fAVDSkillEnergyWeaponsBase", // energy weapons
    "fAVDSkillExplosivesBase", // explosives
    "fAVDSkillLockpickBase", // lockpick
    "fAVDSkillMedicineBase", // medicine
    ...
    "fAVDSkillRepairBase", // repair
    "fAVDSkillScienceBase", // science
    "fAVDSkillSmallGunsBase", // guns/smallguns
    "fAVDSkillSneakBase", // sneak
    "fAVDSkillSpeechBase", // speech
    "fAVDSkillThrowingBase", // survival/throwing(?)
    "fAVDSkillUnarmedBase", // unarmed
};
...
template<class T>
void forEachFollowingPackage(const std::list<Mechanics::Actor>& actors, const MWWorld::Ptr& actorPtr, const MWWorld::Ptr& player, T&& func)
{
    for (const Mechanics::Actor& actor : actors)
    {
        const MWWorld::Ptr &iteratedActor = actor.getPtr();
        if (iteratedActor == player || iteratedActor == actorPtr)
            continue;

        const auto &stats = iteratedActor.getClass().getFOStats(iteratedActor);
        if (stats.mDead)
            continue;
        ...
        for (const auto& package : stats.mAiSequence)
        {
...
namespace F3Mechanics
{
    float calcBaseSkill(const MWWorld::Ptr& ptr, F3Mechanics::Skill skill)
    {
        ...
        if (const auto* avdNv = settings.search(skillAVDSettingNV[skill])) // it's a nv skill
        {
            ...
            return fAVDSkillBase + (stats.mStats[statSkillUses[skill]].getModified() * fAVDSkillPrimaryBonusMult) + std::floor(stats.mStats[F3Mechanics::Stat_Luck].getModified() * fAVDSkillLuckBonusMult);
        }
        else // it's a fo3 skill
        {
            const auto* avdF3 = settings.find(skillAVDSettingFO3[skill]);
            float fAVDSkillBase = avdF3->mFloatValue;
            return fAVDSkillBase + (stats.mStats[statSkillUses[skill]].getModified() * fAVDSkillPrimaryBonusMult) + std::floor(stats.mStats[F3Mechanics::Stat_Luck].getModified() * fAVDSkillLuckBonusMult);
        }
    }
    ...
    namespace 
    {
        float getTimeToDestination(const MWWorld::Ptr& actor, const AiPackage& package, const osg::Vec3f& position, float speed, float duration, const osg::Vec3f& halfExtents)
        {
            const auto distanceToNextPathPoint = (package.getNextPathPoint(package.getDestination(actor)) - position).length();
            ...
            MWWorld::Ptr& headTrackTarget, float& sqrHeadTrackDistance, bool inCombatOrPursue)
        {
            ...
            if (!targetActor.getClass().hasFormId())
            {
                // likely not a fo3 actor
                return;
            }

            if (targetActor.getClass().getFOStats(targetActor).mDead)
                return;

            static const float fMaxHeadTrackDistance = /*MWBase::Environment::get().getWorld()->getStore()
                .get<ESM4::GameSetting>().find("fAIMaxHeadTrackDistance")->mFloatValue*/100.f;
            float maxDistance = fMaxHeadTrackDistance;
            const ESM4::Cell* currentCell = actor.getCell()->getCell4();
            ...
            if ((actorDirection * targetDirection > 0 || inCombatOrPursue)
                ...
                && MWBase::Environment::get().getMechanicsManager("FO3")->awarenessCheck(targetActor, actor))
            {
        ...
        void updateHeadTracking(const MWWorld::Ptr& ptr, const std::list<Actor>& actors, bool isPlayer, CharacterController& ctrl)
        {
            ...
            const auto& stats = ptr.getClass().getFOStats(ptr);
            ...
            if (!stats.mKnockdown && !firstPersonPlayer)
            {
                bool inCombatOrPursue = stats.mAiSequence.isInCombat();
                if (inCombatOrPursue)
                {
                    auto activePackageTarget = stats.mAiSequence.getActivePackage().getTarget();
        ...
        void updateLuaControls(const MWWorld::Ptr& ptr, bool isPlayer, MWBase::LuaManager::ActorControls& controls)
        {
            ...
            auto& stats = ptr.getClass().getFOStats(ptr);
            ...
            const bool runFlag = stats.mMovementFlags & Stats::Movement_Run;
            const bool attackingOrSpell = stats.mAttacking;
            if (controls.mChanged)
            {
                ...
                if (controls.mRun)
                {
                    stats.mMovementFlags |= Stats::Movement_Run;
                } else stats.mMovementFlags &= ~Stats::Movement_Run;
                stats.mAttacking = ((controls.mUse & 1) == 1);
    ...
    {
        if (!actor.getClass().isActor() || actor == MWMechanics::getPlayer() || MWBase::Environment::get().getSoundManager()->sayActive(actor))
            return;

        const Stats &stats = actor.getClass().getFOStats(actor);
        if (stats.mActorValues.at("Hello").getModified() == 0)
            return;

        const AiSequence& seq = stats.mAiSequence;
        if (seq.isInCombat() || seq.hasPackage(AiPackageTypeId::Follow) || seq.hasPackage(AiPackageTypeId::Escort))
            return;

        const osg::Vec3f playerPos(MWMechanics::getPlayer().getRefData().getPosition().asVec3());
        ...
        static const float fIdleChatterCommentTimer = world->getStore().get<ESM4::GameSetting>().find("fIdleChatterCommentTimer")->mFloatValue * 1000;
        static Misc::DeviatingPeriodicTimer timer(fIdleChatterCommentTimer, 0.1f, Misc::Rng::deviate(0, 0.1f, world->getPrng()));
        if (timer.update(10, world->getPrng()) == Misc::TimerStatus::Elapsed)
            MWBase::Environment::get().getDialogueManager("FO3")->say(actor, "IdleChatter");
    }
    ...
    void Actors::updateMovementSpeed(const MWWorld::Ptr& actor) const
    {
        ...
        const Stats &stats = actorClass.getFOStats(actor);
        const AiSequence& seq = stats.mAiSequence;

        if (!seq.isEmpty() && seq.getActivePackage().useVariableSpeed())
        {
            const osg::Vec3f targetPos = seq.getActivePackage().getDestination(actor);
            const osg::Vec3f actorPos = actor.getRefData().getPosition().asVec3();
    ...
    {
        const auto& actorClass = actor.getClass();
        if (!actorClass.isActor() || actor == MWMechanics::getPlayer())
            return;

        const Stats& actorStats = actorClass.getFOStats(actor);
        const AiSequence& seq = actorStats.mAiSequence;
            ...
            (packageId != AiPackageTypeId::Wander && packageId != AiPackageTypeId::Travel && packageId != AiPackageTypeId::None))
        {
            ...
            actorState.setGreetingState(MWMechanics::Greet_None);
            return;
        }
        ...
        if (actorState.isTurningToPlayer())
        {
            ...
            // animation subsystem
            if (MWMechanics::zTurn(actor, actorState.getAngleToPlayer(), osg::DegreesToRadians(5.f)))
            {
                actorState.setTurningToPlayer(false);
                playAnimationGroup(actor, "idle", 0, std::numeric_limits<int>::max(), false);
            }
        }
        ...
        static const int iGreetDistanceMultiplier = 1;

        const float helloDistance = static_cast<float>(actorStats.mActorValues.at("Hello").getModified() * iGreetDistanceMultiplier);
        //const auto& playerStats = player.getClass().getFOStats(player);

        int greetingTimer = actorState.getGreetingTimer();
        MWMechanics::GreetingState greetingState = actorState.getGreetingState();
        if (greetingState == MWMechanics::Greet_None)
        {
            if ((playerPos - actorPos).length2() <= helloDistance * helloDistance
                && MWBase::Environment::get().getWorld()->getLOS(player, actor)
                && MWBase::Environment::get().getMechanicsManager("FO3")->awarenessCheck(player, actor))
                greetingTimer++;

            if (greetingTimer >= GREETING_SHOULD_START)
            {
                greetingState = MWMechanics::Greet_InProgress;
                MWBase::Environment::get().getDialogueManager("FO3")->say(actor, "hello");
                greetingTimer = 0;
            }
        }
        ...
        if (greetingState == MWMechanics::Greet_InProgress)
        {
            ...
            {
                greetingState = MWMechanics::Greet_Done;
                greetingTimer = 0;
            }
        }

        if (greetingState == MWMechanics::Greet_Done)
        {
            float resetDist = 2 * helloDistance;
            if ((playerPos - actorPos).length2() >= resetDist * resetDist)
                greetingState = MWMechanics::Greet_None;
        }
    ...
    {
        auto& ai = ptr.getClass().getFOStats(ptr).mAiSequence;
        ...
        if(ai.getCombatTargets(targets))
        {
            ...
            for(const auto& ally : allies)
                ally.getClass().getFOStats(ally).mAiSequence.stopCombat(targets);
            for(const auto& target : targets)
                target.getClass().getFOStats(target).mAiSequence.stopCombat(allies);
        }
    }
        ...
        std::map<const MWWorld::Ptr, const std::set<MWWorld::Ptr>>& cachedAllies, bool againstPlayer) const
    {
        ...
        Stats& creatureStats1 = actor1.getClass().getFOStats(actor1);
        if (creatureStats1.mDead || creatureStats1.mAiSequence.isInCombat(actor2))
            return;

        const Stats& creatureStats2 = actor2.getClass().getFOStats(actor2);
        if (creatureStats2.mDead)
            return;
        ...
        const auto mechanicsManager = MWBase::Environment::get().getMechanicsManager("FO3");
        ...
        {
            if (creatureStats1.mAiSequence.isInCombat(ally))
                continue;

            if (creatureStats2.mFormId == (ally.getClass().getFOStats(ally).mLastHitAttemptObject))
            {
                ...
                creatureStats1.mLastHitAttemptObject = (ally.getClass().getFOStats(ally).mLastHitAttemptObject);
                return;
            }

            // If there's been no attack attempt yet but an ally of actor1 is in combat with actor2, become aggressive to actor2
            if (ally.getClass().getFOStats(ally).mAiSequence.isInCombat(actor2))
                aggressive = true;
        }
        ...
        if (!aggressive && !isPlayerFollowerOrEscorter)
        {
            ...
            if (creatureStats2.mAiSequence.isInCombat(actor1))
            {
                ...
                {
                    if (ally2.getClass().getFOStats(ally2).mAiSequence.isInCombat(actor1))
                    {
                        mechanicsManager->startCombat(actor1, actor2);
        ...
        // Stop here if target is unreachable (todo)
        ...
        if (!aggressive && isPlayerFollowerOrEscorter && followersAttackOnSight)
        {
            ...
            else
            {
                for (const MWWorld::Ptr& ally : allies1)
                {
                    if (creatureStats2.mAiSequence.isInCombat(ally))
                    {
                        aggressive = true;
                        break;
                    }
    ...
    {
        Stats& creatureStats = creature.getClass().getFOStats (creature);
        const bool wasDead = creatureStats.mDead;

        // TODO: status effects (there's no magic in fallout, but the status effects use the same general logic as TES's magic effects)
    ...
    static void updateDrowning(const MWWorld::Ptr& ptr, float duration, bool isKnockedOut, bool isPlayer)
    {
        ...
        Stats& stats = actorClass.getFOStats(ptr);
    ...
    Actors::Actors() : Mechanics::Actors()
    {
    ...
    bool Actors::isActorDetected(const MWWorld::Ptr& actor, const MWWorld::Ptr& observer) const
    {
        ...
        if (!observer.isEmpty() && observer.getClass().isNpc())
        {
            return
                ...
                MWBase::Environment::get().getMechanicsManager("FO3")->awarenessCheck(actor, observer);
        }
        ...
        for (const MWWorld::Ptr &neighbor : neighbors)
        {
            ...
            const bool result = MWBase::Environment::get().getWorld()->getLOS(neighbor, actor)
                       && MWBase::Environment::get().getMechanicsManager("FO3")->awarenessCheck(actor, neighbor);
    ...
    void Actors::updateCombatMusic ()
    {
        ...
        const bool aiActive = MWBase::Environment::get().getMechanicsManager("FO3")->isAIActive();
        ...
        if (aiActive)
        {
            for (const Actor& actor : mActors)
            {
                ...
                {
                    Stats& stats = actor.getPtr().getClass().getFOStats(actor.getPtr());
                    if (!stats.mDead && stats.mAiSequence.isInCombat())
                    {
                        hasHostiles = true;
                        break;
                    }
        ...
        if (mCurrentMusic != MusicType::Explore && !hasHostiles
                && !(/*player.getClass().getFOStats(player).mDead
    ...
    {
        if (!MWBase::Environment::get().getMechanicsManager("FO3")->isAIActive())
            return;
        ...
        for (const Actor& actor : mActors)
        {
            ...
            const auto& aiSequence = ptr.getClass().getFOStats(ptr).mAiSequence;
            if (!aiSequence.isEmpty())
            {
                ...
                else if ((package.getTypeId() == AiPackageTypeId::Wander || package.getTypeId() == AiPackageTypeId::Sandbox) && giveWayWhenIdle)
                {
                    if (!static_cast<const AiSandbox&>(package).isStationary())
                        shouldGiveWay = true;
                }
                else if (package.getTypeId() == AiPackageTypeId::UseWeapon || package.getTypeId() == AiPackageTypeId::Ambush)
                {
            ...
            if (!shouldGiveWay && !aiSequence.isEmpty())
                timeToCheck = std::min(timeToCheck, getTimeToDestination(ptr, **aiSequence.begin(), basePos, maxSpeed, duration, halfExtents));
            ...
            for (const Actor& otherActor : mActors)
            {
                ...
                if (!MWBase::Environment::get().getWorld()->getLOS(otherPtr, ptr))
                    continue;
                if (!MWBase::Environment::get().getMechanicsManager("FO3")->awarenessCheck(otherPtr, ptr))
                    continue;
                ...
                if (otherPtr.getClass().getFOStats(otherPtr).mDead)
            ...
            if (timeToCollision < timeToCheck)
            {
                ...
                if (shouldTurnToApproachingActor)
                    MWMechanics::zTurn(ptr, angleToApproachingActor);
            }
        }
    }
    void Actors::update (float duration, bool paused)
    {
        if(!paused)
        {
            ...
            const bool aiActive = MWBase::Environment::get().getMechanicsManager("FO3")->isAIActive();
            /*const ESM4::FormId attackedByPlayerId = player.getClass().getFOStats(player).mLastHitAttemptObject;
            if (attackedByPlayerId != 0)
            {
                const MWWorld::Ptr playerHitAttemptActor = world->searchPtrViaActorId(attackedByPlayerId);

                if (!playerHitAttemptActor.isInCell())
                    player.getClass().getFOStats(player).mLastHitAttemptObject = 0;
            }*/
            ...
            for (Actor& actor : mActors)
            {
                ...
                if (isPlayer)
                    continue; // ignore the morrowind player for now
                ...
                if (!isPlayer && (actor.getPtr().getClass().getFOStats(actor.getPtr()).mDead
                    || !actor.getPtr().getClass().getFOStats(actor.getPtr()).mAiSequence.isInCombat()
                    || !inProcessingRange))
                {
                    // todo: avoid softlocks that happen when it tries to perform checks on the morrowind player
                    /*actor.getPtr().getClass().getFOStats(actor.getPtr()).mLastHitAttemptObject = 0;
                    if (player.getClass().getFOStats(player).mLastHitAttemptObject == actor.getPtr().getClass().getFOStats(actor.getPtr()).mFormId)
                        player.getClass().getFOStats(player).mLastHitAttemptObject = 0;*/
                }
                ...
                if (actor.getPtr().getClass().getFOStats(actor.getPtr()).mDead)
                {
                    // They can be added during the death animation
                    if (!actor.getPtr().getClass().getFOStats(actor.getPtr()).mDeathAnimationFinished)
                ...
                else
                {
                    ...
                    if (aiActive && inProcessingRange)
                    {
                        ...
                        if (!isPlayer)
                        {
                            ...
                            {
                                stats.mAiSequence.execute(actor.getPtr(), ctrl, duration);
                    ...
                    {
                        Stats &stats = actor.getPtr().getClass().getFOStats(actor.getPtr());
                        stats.mAiSequence.execute(actor.getPtr(), ctrl, duration, /*outOfRange*/true);
                    }
            ...
            for (Actor& actor : mActors)
            {
                ...
                if (!isPlayer && isConscious(actor.getPtr()))
                {
                    AiSequence& seq = stats.mAiSequence;
                ...
                world->setActorCollisionMode(actor.getPtr(), true, !actor.getPtr().getClass().getFOStats(actor.getPtr()).mDeathAnimationFinished);
            ...
            for (const Actor& actor : mActors)
            {
                if (actor.getPtr() == player)
                    continue; // ignore morrowind character for now
                const MWWorld::Class &cls = actor.getPtr().getClass();
                Stats &stats = cls.getFOStats(actor.getPtr());

                //KnockedOutOneFrameLogic
                //Used for "OnKnockedOut" command
                //Put here to ensure that it's run for PRECISELY one frame.
                if (stats.mKnockdown && !stats.mKnockdownOneFrame && !stats.mKnockdownOverOneFrame)
                { //Start it for one frame if nessesary
                    stats.mKnockdownOneFrame = (true);
                }
                else if (stats.mKnockdownOneFrame && !stats.mKnockdownOverOneFrame)
                { //Turn off KnockedOutOneframe
                    stats.mKnockdownOneFrame = (false);
                    stats.mKnockdownOverOneFrame = (true);
                }
            }
    ...
    {
        actor.getClass().getFOStats(actor).mDied = true;
    ...
    void Actors::killDeadActors()
    {
        for (Actor& actor : mActors)
        {
            if (actor.getPtr() == MWMechanics::getPlayer())
                continue; // ignore morrowind player for now
            const MWWorld::Class &cls = actor.getPtr().getClass();
            auto &stats = cls.getFOStats(actor.getPtr());
            ...
            if (killResult == CharacterController::Result_DeathAnimStarted)
            {
                ...
                MWBase::Environment::get().getDialogueManager("FO3")->say(actor.getPtr(), "death");
            ...
            {
                const bool isPlayer = actor.getPtr() == MWMechanics::getPlayer();
                if (isPlayer)
                    continue; // ignore the morrowind player for now
                notifyDied(actor.getPtr());
                
                // Make sure spell effects are removed
                purgeSpellEffects(stats.mActorId);
                ...
                if (isPlayer)
                {
                    ...
                    MWBase::Environment::get().getSoundManager()->streamMusic("Special/Death.mp3");
                }
                else
                {
    ...
    void Actors::updateSneaking(CharacterController* ctrl, float duration)
    {
        ...
        // if (!MWBase::Environment::get().getMechanicsManager("FO3")->isSneaking(player))
        // {
        //     MWBase::Environment::get().getWindowManager()->setSneakVisibility(false);
        //     return;
        // }
        ...
        //         {
        //             if (MWBase::Environment::get().getMechanicsManager("FO3")->awarenessCheck(player, observer))
        //             {
        //                 detected = true;
        //                 avoidedNotice = false;
        //                 MWBase::Environment::get().getWindowManager()->setSneakVisibility(false);
    ...
    std::vector<MWWorld::Ptr> Actors::getActorsSidingWith(const MWWorld::Ptr& actorPtr, bool excludeInfighting) const
    {
        ...
        {
            const MWWorld::Ptr& iteratedActor = actor.getPtr();
            if (iteratedActor == MWMechanics::getPlayer())
                continue;

            const bool sameActor = (iteratedActor == actorPtr);

            const Stats &stats = iteratedActor.getClass().getFOStats(iteratedActor);
            if (stats.mDead)
                continue;
            ...
            for (const auto& package : stats.mAiSequence)
            {
                if (excludeInfighting && !sameActor && package->getTypeId() == AiPackageTypeId::UseWeapon && package->getTarget() == actorPtr)
                    break;
                if (package->sideWithTarget() && !package->getTarget().isEmpty())
                {
                    if (sameActor)
                    {
                        if(excludeInfighting)
                        {
                            ...
                            if(ally.getClass().getFOStats(ally).mAiSequence.getCombatTargets(enemies)
    ...
    {
        std::vector<MWWorld::Ptr> list;
        forEachFollowingPackage(mActors, actorPtr, MWMechanics::getPlayer(), [&] (const Actor& actor, const std::shared_ptr<AiPackage>& package)
        {
            if (package->followTargetThroughDoors() && package->getTarget() == actorPtr)
                list.push_back(actor.getPtr());
            else if (package->getTypeId() != AiPackageTypeId::UseWeapon && package->getTypeId() != AiPackageTypeId::Wander && package->getTypeId() != AiPackageTypeId::Sandbox)
                return false;
            return true;
        });
        return list;
    ...
    {
        std::vector<int> list;
        forEachFollowingPackage(mActors, actor, MWMechanics::getPlayer(), [&] (const Actor&, const std::shared_ptr<AiPackage>& package)
        {
    ...
    std::map<int, MWWorld::Ptr> Actors::getActorsFollowingByIndex(const MWWorld::Ptr &actor) const
    {
        ...
        forEachFollowingPackage(mActors, actor, MWMechanics::getPlayer(), [&] (const Actor& otherActor, const std::shared_ptr<AiPackage>& package)
        {
            if (package->followTargetThroughDoors() && package->getTarget() == actor)
            {
                // todo
    ...
    std::vector<MWWorld::Ptr> Actors::getActorsFighting(const MWWorld::Ptr& actor) const
    {
        ...
        for(const MWWorld::Ptr& neighbor : neighbors)
        {
            ...
            const Stats &stats = neighbor.getClass().getFOStats(neighbor);
            if (stats.mDead)
                continue;

            if (stats.mAiSequence.isInCombat(actor))
    ...
    std::vector<MWWorld::Ptr> Actors::getEnemiesNearby(const MWWorld::Ptr& actor) const
    {
        ...
        {
            const Stats &stats = neighbor.getClass().getFOStats(neighbor);
            if (stats.mDead || neighbor == actor || neighbor.getClass().isPureWaterCreature(neighbor))
                continue;

            const bool isFollower = followers.find(neighbor) != followers.end();

            if (stats.mAiSequence.isInCombat(actor)
                    || (MWBase::Environment::get().getMechanicsManager("FO3")->isAggressive(neighbor, actor) && !isFollower))
                list.push_back(neighbor);
        }
        return list;
    }
    ...
    MWMechanics::GreetingState Actors::getGreetingState(const MWWorld::Ptr& ptr) const
    {
        const auto it = mIndex.find(ptr.mRef);
        if (it == mIndex.end())
            return MWMechanics::Greet_None;
    ...
    void Actors::fastForwardAi() const
    {
        ...
        for (auto it = mActors.begin(); it != mActors.end();)
        {
            ...
            if (ptr == MWMechanics::getPlayer()
                    || !isConscious(ptr))
                continue;
            AiSequence& seq = ptr.getClass().getFOStats(ptr).mAiSequence;
            seq.fastForward(ptr);
        }
    }
}
