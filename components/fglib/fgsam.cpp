...
namespace FgLib
{
        ...
        const std::vector<float>& npcAsymCoeff, const VFS::Manager* manager) const
    {
        ...
        const FgTri* tri = triFile.getOrLoadByMeshName(nif, manager);
...
//        }
        ...
        bool hat, const VFS::Manager* manager) const
    {
...
#if defined(__GNUC__) && __GNUC__ < 8
            tri = triFile.replaceFile(Misc::StringUtils::lowerCase(name),
        ...
        FgFile<FgEgm> egmFile;
        const FgEgm* egm = egmFile.getOrLoadByName(nif.substr(0, pos) + (hat ? "hat.egm" : "nohat.egm"), manager);
    ...
    bool FgSam::buildMorphedVerticesImpl(std::vector<osg::Vec3f>& fgMorphVertices,
        const FgEgm* egm,
        const FgTri* tri,
        const std::vector<float>& raceSymCoeff,
        const std::vector<float>& raceAsymCoeff,
    ...
    float FgSam::getAge(const std::vector<float>& raceSymCoeff, const std::vector<float>& npcSymCoeff, const VFS::Manager* manager) const
    {
        FgFile<FgCtl> ctlFile;
        const FgCtl* ctl = ctlFile.getOrLoadByName("facegen\\si.ctl", manager); // WARN: hard-coded path
    ...
    std::string FgSam::getTES4NpcDetailTexture_0(const std::string& esmName, const std::string& npcFormIdString, VFS::Manager* manager) const
    {
        // FIXME: need to be able to check other than oblivion.esm - can this hard-coded path change?
        /// TODO: use esm name instead of "oblivion.esm"
        std::string textureFile = "textures\\faces\\" + esmName + "\\" + npcFormIdString + "_0.dds ";
        if (manager->exists(textureFile))
            return textureFile;
    ...
    std::string FgSam::getFO3NpcDetailTexture_0(const std::string& esmName, const std::string& npcFormIdString, VFS::Manager* manager) const
    {
        std::string textureFile = "textures\\characters\\facemods\\" + esmName + "\\" + npcFormIdString + "_0.dds";
        if (manager->exists(textureFile))
            return textureFile;
    ...
    std::string FgSam::getFONVNpcDetailTexture_0(const std::string& esmName, const std::string& npcFormIdString, VFS::Manager* manager) const
    {
        std::string textureFile = "textures\\characters\\facemods\\" + esmName + "\\" + npcFormIdString + "_0.dds";
        if (manager->exists(textureFile))
            return textureFile;
    ...
    std::string FgSam::getTES5NpcDetailTexture_0(const std::string& esmName, const std::string& npcFormIdString, VFS::Manager* manager) const
    {
        std::string textureFile = "textures\\actors\\facemods\\" + esmName + "\\"  +npcFormIdString + "0.dds";
        if (manager->exists(textureFile))
            return textureFile;
    ...
    osg::ref_ptr<osg::Texture2D> FgSam::getMorphedTexture(
        ...
        const std::vector<float>& npcSymCoeff, Resource::ResourceSystem* manager) const
    {
        ...
        const FgEgt* egt = egtFile.getOrLoadByMeshName(mesh, manager->getVFS());

        if (egt == nullptr)
            return nullptr; // texture morph not possible

        return getMorphedTexture(egt, texture, npcName, raceSymCoeff, npcSymCoeff, manager);
    }

    osg::ref_ptr<osg::Texture2D> FgSam::getMorphedTES4BodyTexture(
        const std::string& mesh,
        const std::string& texture,
        const std::string& npcName,
        const std::vector<float>& raceSymCoeff,
        const std::vector<float>& npcSymCoeff, Resource::ResourceSystem* manager) const
    {
        std::string meshName = Misc::StringUtils::lowerCase(mesh);
        ...
        FgFile<FgEgt> egtFile;
        const FgEgt* egt = egtFile.getOrLoadByMeshName(meshName, manager->getVFS());
 ...
 */
        ...
        const std::vector<float>& npcSymCoeff, Resource::ResourceSystem* manager) const
    {
        ...
        osg::ref_ptr<osg::Image> image = manager->getImageManager()->getImage(texture);
        //baseTexture->load();
        ...
        uint8_t* pDest = image->data();

        osg::Vec3f sym;
        const std::vector<osg::Vec3f>& symTextureModes = egt->symTextureModes();
        ...
        for (size_t i = 0; i < egt->numRows() * egt->numColumns(); ++i) // height*width, should be 256*256
        {
            ...
            if (npcSymCoeff.empty())
            {
                ...
                for (size_t j = 0; j < numSymTextureModes; ++j)
                    sym += symTextureModes[numSymTextureModes * i + j] * raceSymCoeff[j];
            }
            else
            {
                for (size_t j = 0; j < numSymTextureModes; ++j)
                    sym += symTextureModes[numSymTextureModes * i + j] * (raceSymCoeff[j] + npcSymCoeff[j]);
            }
